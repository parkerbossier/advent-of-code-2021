...
function main1(numbers: number[], boards: Board[]) {
	const markBoards: Board<boolean>[] = boards.map(b => {
		return b.map(r => {
...
function main2(numbers: number[], boards: Board[]) {
	const markBoards: Board<boolean>[] = boards.map(b => {
		return b.map(r => {
			return new Array(b.length).fill(false);
		});
	...
	let boardsAlreadyWon: number[] = [];
	for (let n = 0; n < numbers.length; n++) {
		const number = numbers[n];

		for (let b = 0; b < boards.length; b++) {
			if (boardsAlreadyWon.includes(b)) continue;
...
function mark(board: Board, markBoard: Board<boolean>, number: number) {
	for (let r = 0; r < board.length; r++) {
		for (let c = 0; c < board.length; c++) {
			if (board[r][c] === number) markBoard[r][c] = true;
		}
	}
}

function checkBoard(markBoard: Board<boolean>) {
	// rows
	if (markBoard.some(r => r.every(Boolean))) return true;
...
function scoreBoard(board: Board, markBoard: Board<boolean>, number: number) {
...
const chunks = day4data.split('\n\n');
const numbers = chunks[0].split(',').map(n => parseInt(n, 10));

const boards = (() => {
	const _boards: Board[] = chunks.slice(1).map(chunk => {
